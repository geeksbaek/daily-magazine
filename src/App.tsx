...
export default function App() {
  ...
  const handleSelectIssue = async (date: string) => {
    await loadMagazine(date); setView('magazine'); window.scrollTo({ top: 0 })
  }
    ...
    return (
      <div className="min-h-screen flex items-center justify-center" style={{ backgroundColor: 'var(--bg)' }}>
        <div className="text-center">
          <div className="font-serif font-black text-[24px] mb-6" style={{ color: 'var(--text)', letterSpacing: '-0.02em' }}>
    ...
    return (
      <div className="min-h-screen flex items-center justify-center" style={{ backgroundColor: 'var(--bg)' }}>
        <div className="text-center max-w-sm px-6">
          ...
          <p className="text-[14px] mb-4 font-sans" style={{ color: 'var(--text-secondary)' }}>{error}</p>
          <button onClick={() => window.location.reload()} className="text-[12px] font-mono underline" style={{ color: 'var(--accent)' }}>다시 시도</button>
        </div>
      </div>
    )
  }
    ...
    <ThemeProvider value={themeId}>
      <div className="magazine-scroll" style={{ backgroundColor: 'var(--bg)' }}>
        ...
        {view === 'archive' && archiveIndex ? (
          <Archive issues={archiveIndex.issues} onSelectIssue={handleSelectIssue}
            onBack={() => { setView('magazine'); window.scrollTo({ top: 0 }) }} />
        ) : magazine ? (
          <>
            <Cover magazine={magazine} onReadIssue={handleReadIssue} />
            {magazine.podcast && <PodcastPlayer podcast={magazine.podcast} date={currentDate} />}
            <Highlights articles={magazine.highlights} />
            <NewsSection id="ai-ml" number="02" title="AI & Machine Learning"
              subtitle="인공지능 · 머신러닝" articles={magazine.sections.ai_ml} accentColor="#7C3AED" bgAlternate />
            <NewsSection id="dev-tools" number="03" title="Developer Tools"
              subtitle="개발 도구 · 플랫폼" articles={magazine.sections.dev_tools} accentColor="#0891B2" />
            ...
            {magazine.sections.threads_pulse && <ThreadsPulse posts={magazine.sections.threads_pulse} />}
